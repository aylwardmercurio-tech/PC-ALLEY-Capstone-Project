  ...
  return (
    <div className="flex bg-brand-bgbase min-h-screen text-main font-dmsans transition-colors duration-300">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="flex-1 overflow-hidden p-4 md:p-8 flex gap-8 bg-brand-bgbase text-main">
          {/* Main Selection Area */}
          <div className="flex-[2] flex flex-col space-y-6 overflow-hidden">
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <button className="h-14 px-6 bg-brand-surface border border-border rounded-2xl flex items-center gap-3 text-[10px] font-black uppercase tracking-[2px] text-muted hover:bg-brand-muted/5 transition-all shadow-sm">
                  ...
                  {filteredInventory.map((item, i) => (
                    ...
                    >
                      <div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Plus size={20} className="text-[#00F2FF]" />
                      </div>
                      <div className="w-16 h-16 rounded-2xl bg-brand-bgbase border border-border flex items-center justify-center mb-6 group-hover:border-brand-neonblue/20 transition-all">
                        <Package size={24} className="text-muted/20 group-hover:text-brand-neonblue" />
                      </div>
                      <h4 className="text-[14px] font-black text-main mb-1 group-hover:text-brand-neonblue transition-colors truncate">{item.name}</h4>
          ...
          {/* Checkout Side Panel */}
          <div className="flex-1 bg-brand-surface border border-border rounded-[40px] flex flex-col relative overflow-hidden shadow-sm">
            {/* Header */}
                ...
                {cart.length > 0 ? (
                  cart.map((item) => (
                    <motion.div
                      ...
                      exit={{ opacity: 0, scale: 0.9 }}
                      className="group p-5 bg-white/5 rounded-3xl border border-white/5 relative"
                    >
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3 p-1.5 bg-black/40 rounded-xl border border-white/5">
            ...
            {/* Calculations & Footer */}
            <div className="p-10 bg-brand-muted/5 border-t border-border space-y-8">
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-end mb-8">
                  ...
                  <span className="text-4xl font-rajdhani font-black text-main tracking-widest">₱{grandTotal.toLocaleString()}</span>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className={`w-full h-18 rounded-[24px] font-black uppercase tracking-[6px] text-xs flex items-center justify-center gap-4 transition-all ${
                      ...
                      : "bg-brand-crimson hover:bg-red-700 text-white shadow-[0_20px_40px_rgba(215,38,56,0.3)] active:scale-[0.98] disabled:opacity-50 disabled:grayscale"
                    }`}
                  >
