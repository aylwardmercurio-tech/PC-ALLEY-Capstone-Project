      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 custom-scrollbar relative z-10">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-brand-surface border border-border rounded-2xl p-6 lg:p-10 shadow-sm flex flex-col items-center justify-center min-h-[400px]">
            <Download size={48} className="text-muted mb-4 opacity-50" />
            <p className="text-muted font-bold text-sm">No Purchases Logged in the system.</p>
            <p className="text-muted/60 text-xs mt-2 uppercase tracking-widest text-center">You can add purchases to populate this list.</p>
          </motion.div>
        </div>
      </main>
    </div>
